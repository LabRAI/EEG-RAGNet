...
from model.eeg_ragnet import EEG_RAGNet



# 在 for x, y, ... 循环内部：
if args.model_name == "evobrain" or args.model_name == "evolvegcn":
    logits, adj_t = model(x, seq_lengths, adj)
elif args.model_name == "dcrnn":
    logits, adj_t, *_ = model(x, seq_lengths, supports)
elif args.model_name == "BIOT":
    logits, adj_t = model(x)
elif args.model_name == "lstm" or args.model_name == "cnnlstm":
    logits, adj_t = model(x, seq_lengths)
else:
    print("model_name: ", args.model_name)
    raise NotImplementedError


#Phase1: save dynamic graph structure
if args.save_graphs:
    os.makedirs(args.graphs_save_dir, exist_ok=True)
    np.save(os.path.join(args.graphs_save_dir, f"adj_epoch{epoch}_step{step}.npy"),
            adj_t.detach().cpu().numpy())

#Phase 1: reserve EEG-RAGNet hook api
if args.use_ragnet:
    adj_t = ragnet_hook.refine_graph(adj_t, x)
...
def main(args):
    ...
    elif args.model_name == "eeg_ragnet":
        model = EEG_RAGNet(args)
    else:
        raise NotImplementedError
...
def train(model, dataloaders, args, device, save_dir, log, tbx):
    ...
    if args.use_ragnet:
        from model.eeg_ragnet.eeg_ragnet import EEG_RAGNet
        ragnet_hook = EEG_RAGNet(
            input_dim=args.semantic_input_dim,
            hidden_dim=args.semantic_hidden_dim,
            proj_dim=args.semantic_proj_dim,
            index_path=args.faiss_index_path,
            kg_json_path=args.kg_triplets_path,
            k=args.topk_retrieval,
            refine_threshold=args.refine_threshold,
            refine_alpha=args.refine_alpha,
            save_path=args.refine_save_path
        ).to(device)
    else:
        ragnet_hook = None
    ...
    while (epoch != args.num_epochs) and (not early_stop):
        ...
        with torch.enable_grad(), \
            ...
            for x, y, seq_lengths, supports, adj, file_name in train_loader:
                ...
                if args.use_ragnet and (step % args.refine_interval == 0):
                    with torch.no_grad():
                        adj = ragnet_hook.refine_graph(adj, x)
                elif args.model_name == "evobrain" or args.model_name == "evolvegcn":
